...
folderPath = 'C:\Users\ms0405\Suite2P\Interaxonal_analysis'; % Update with your folder path
    ...
    if exist(filename, 'file') == 2
        try
            ...
            for i = 1:size(scaledCorrelationMatrix, 1)
                scaledCorrelationMatrixNoDiagonal(i, i) = NaN;
            end
            
            % Calculate the average correlation per bouton excluding the diagonal
            meanPositiveCorrelationsPerBouton = nanmean(scaledCorrelationMatrixNoDiagonal, 2);


            % STEP 7: EXPORT RESULTS
            fileColumn = repmat({baseFileName}, numColumns, 1);
            BoutonResults = table((1:numColumns)', meanPositiveCorrelationsPerBouton, averagePks, Frequency, fileColumn, ...
                'VariableNames', {'Bouton', 'AveragePositiveCorrelation', 'Amplitude', 'Frequency', 'File_ID'});
            outputFileName = fullfile(folderPath, [baseFileName, '_AxonalResults.xlsx']);
            writetable(BoutonResults, outputFileName);
